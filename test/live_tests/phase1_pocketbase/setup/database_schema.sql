...
-- Generated on: 2025-08-14T10:22:50.549917
-- Table: usm_test
...
CREATE TABLE IF NOT EXISTS usm_test (
    ...
    -- Local UUID for tracking records across sync operations
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_usm_test_org_id ON usm_test (organizationId);
CREATE INDEX IF NOT EXISTS idx_usm_test_category_active ON usm_test (testCategory, isActive);
CREATE INDEX IF NOT EXISTS idx_usm_test_sync_status ON usm_test (isDirty, isDeleted);
CREATE INDEX IF NOT EXISTS idx_usm_test_created ON usm_test (createdAt);
CREATE INDEX IF NOT EXISTS idx_usm_test_updated ON usm_test (updatedAt);